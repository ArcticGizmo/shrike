...
>
> **Storage (2026-08-17).** Recordings + their sidecars moved off `%TEMP%` (which the OS can purge) to a stable per-profile working folder, `%LOCALAPPDATA%\Shrike\recordings` (dev = `Shrike (Dev)`), via the new `AppStorage` helper — so a clip survives to be edited / re-exported. A **Debug-only** tray item, *Open working folder (debug)*, opens it for inspection. Retention is bounded by `RecordingsRetention` (keep the newest; evict past 20 files / 2 GB / 14 days, deleting each recording with its sidecar + clearing orphans), swept at startup and after each recording.
...
> **Plan complete (SC1–SC5).** Record → smoothed synthetic cursor → click ripples → auto-zoom, all through one managed decode→compose→encode pass, tunable and previewed live.

> **Un-gated + preset parity (2026-08-18).** The feature is now available in Release (opt-in via the **Smooth cursor** HUD toggle, off by default) — the `#if DEBUG` guards around the toggle, preview overlay, and tuning panel were removed (the file-inspection buttons stay dev-only). **Preset parity** via a two-stage export: the composite pass renders the cursor + zoom into a **high-quality intermediate** at the target size, then the normal `ExportCommand`/`VideoExporter` encodes that intermediate to the chosen preset — so **every** preset (H.265 / hardware / GIF / WebP / Source) ships with the cursor baked in, reusing all the existing encode logic. The editor's tuned smoothing + zoom carry into the export. Test: `Composite_then_export_honours_the_chosen_preset` (composite → H.265 → valid clip of the right length). Trade-offs (noted): two encodes (a generous-bitrate intermediate keeps loss ~transparent) and the zoom resample is managed CPU (fine offline). Full suite 225 green; Debug + Release clean.
