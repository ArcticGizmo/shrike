...
>
> **Preview overlay (debug).** To *see* the smoothing before the export pipeline exists, the timeline editor now overlays the projected cursor on the preview during Play/scrub (`PreviewSurface.SetCursor`, positioned in the Uniform-fit rect; re-projected on edits so it respects cuts). Debug-only for now — showing it in preview but not in the export would break WYSIWYG, so it's un-gated once **SC4** draws the cursor into exported frames too. Next: **SC3** (the decode → composite → re-encode pipeline, proven with a no-op compositor), which will consume this per-frame track.
