...
- ◧ **M6.3 — release infra + branding (configured; awaiting repo + tag).**
>   - **Branding** ✅ — `tools/IconGen` (SVG→PNG/ICO, mirrors perch) + `tools/gen-icons.ps1` render `shrike.svg` into `shrike.ico` (wired as the `.exe` `ApplicationIcon`), `Assets/icon.png`, and `shrike-icon.png` (README header, now in the README).
>   - **ffmpeg bundle** ✅ (mechanism) — `tools/fetch-ffmpeg.ps1` downloads a **pinned, hash-verified** prebuilt GPL ffmpeg into the publish folder next to `shrike.exe`; the release workflow runs it before `vpk pack`. Decision (agreed): **pinned prebuilt, manually bumpable** (best for GitHub Actions vs. a fragile source build); it's a ~150 MB full static exe for now, swappable for a hand-built ~30–50 MB lean exe at the same path with no code change.
>   - **Release automation** ✅ — `.github/workflows/release.yml` (tag-triggered publish → fetch-ffmpeg → `vpk pack` → `SHA256SUMS.txt` → GitHub Release) + a pure-ASCII `install.ps1` one-liner (checksum-verified), both mirroring sprig. Unsigned (agreed).
>   - **QA gates** — kept **soft** by request (the startup-budget gate already lives in `BudgetEvaluator` tests; the heavier memory/throughput/clipboard/DPI matrices are deferred).
>   - **Remaining (yours):** create the GitHub repo, then swap the `TODO(release)` feed URL / confirm the `quartexsoftware/shrike` slug; pin the ffmpeg `$Sha256`; push a `v1.0.0` tag to cut the release.
