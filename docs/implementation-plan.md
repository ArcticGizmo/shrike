...
>   - **Branding** ✅ — `tools/IconGen` (SVG→PNG/ICO, mirrors perch) + `tools/gen-icons.ps1` render `shrike.svg` into `shrike.ico` (wired as the `.exe` `ApplicationIcon`), `Assets/icon.png`, and `landing-icon.png` (README header, now in the README).
...
>   - **Remaining (yours):** create the `ArcticGizmo/shrike` GitHub repo; pin the ffmpeg `$Sha256`; push a `v1.0.0` tag to cut the release.
