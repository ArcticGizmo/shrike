...
>
> **v1 limitations (GDI capture):** the HUD is placed outside the region to stay out of frame, but a **full-screen** recording can still catch it; GDI grabs only what's composited (no exclusive-fullscreen/DX content). Both go away with the WGC `IFrameSource` upgrade. ffmpeg must be **bundled** at packaging (M6); until then the app finds it via `SHRIKE_FFMPEG` / PATH.
